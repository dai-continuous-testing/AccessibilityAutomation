...
public final class ContrastChecker {
    ...
    public static ContrastResult findContrast(BufferedImage image){
        MMCQ.CMap result = ColorThief.getColorMap(image, 2);
        int[] rgbFirst = result.vboxes.get(0).avg(false);
        int[] rgbSecond = result.vboxes.get(1).avg(false);
        Color c1 = new Color(rgbFirst[0], rgbFirst[1], rgbFirst[2]);
        Color c2 = new Color(rgbSecond[0], rgbSecond[1], rgbSecond[2]);
        double contrast = ContrastChecker.getConstrastRatio(c1, c2);
        ContrastResult contrastResult = new ContrastResult();
        contrastResult.setColor1(c1);
        contrastResult.setColor2(c2);
        contrastResult.setRatio(contrast);
        contrastResult.setImage(image);
